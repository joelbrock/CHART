...
if (($_POST['submit'])) {
	$cols[]='`content-1`';$vals[]=mysql_escape_string($_REQUEST['content-1']);
	$cols[]='`content-2`';$vals[]=mysql_escape_string($_REQUEST['content-2']);
	$cols[]='`content-3`';$vals[]=mysql_escape_string($_REQUEST['content-3']);
	$cols[]='`intro_default`';$vals[]=mysql_escape_string($_REQUEST['intro_default']);
	if(!empty($report)){
...
<div id="wrapper">
	<div id="client_profile">
		<form method='POST' action='<?=$_SERVER['PHP_SELF']?>' style="width:680px;" class='aligncenter' autocomplete="off">


		<div class="form_field">
			<h3>Default intro text</h3>
			<textarea name="intro_default" cols="80" rows="3"><?=@$report['intro_default']?></textarea>
		</div>
		<br>
		...
		<div class="form_field" style="float:right; margin-left:0;">
			...
			<textarea name="content-1" cols="60" rows="10"><?=@$report['content-1']?></textarea>
		</div>
		<div class="form_field" style="float:right; margin-left:0;">
			<h3>Content Block 2</h3>
			<textarea name="content-2" cols="60" rows="10"><?=@$report['content-2']?></textarea>
		</div>
		<div class="form_field" style="float:right; margin-left:0;">
				<h3>Content Block 3</h3>
				<textarea name="content-3" cols="60" rows="10"><?=@$report['content-3']?></textarea>
				<br><br>
