...
DEFINE ('DB_USER', 'cbldapp');
DEFINE ('DB_PASSWORD', 'CBLDpa55');
DEFINE ('DB_HOST', 'mysql.designslife.com');
DEFINE ('DB_NAME', 'cds_coop');
...
// function StaffMenu() { // -- STAFFMENU START
// 	$staffr = mysql_query("SELECT * FROM staff");
// 	$out = "<select name=\"staffmenu\">\n";
// 	while ($staff = (mysql_fetch_assoc($staffr))) {
// 		$fullname = $staff['firstname'] . " " . $staff['lastname'];
// 		$out .= "<option value=\"" . $staff['id'] . "\" ";
// 		if ($staff['id'] == $_POST['staffmenu']) { $out .= "SELECTED"; }
// 		$out .= ">" . $fullname;
// 		$out .= "</option>\n";
// 	}
// 	$out .= "</select>\n";
...
// } // -- STAFFMENU ENDS
// 
// function CoopMenu () {	// -- CODEMENU START
// 	$coder = mysql_query("SELECT * FROM cococodes");
// 	$out =  "<select name=\"codemenu\">\n";
// 	while ($code = (mysql_fetch_assoc($coder))) {
// 		$out .= "<option value=\"" . $code['id'] . "\">" . $code['name'];
// 		if ($code['code']) {
// 			$out .= " - " . $code['code'];
// 		}
// 		$out .= "</option>\n";
