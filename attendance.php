...
$get_c=mysql_query("SELECT * FROM clients WHERE active = 1 ORDER BY name ASC") or die(mysql_error());
while($c=mysql_fetch_assoc($get_c)) $c_meta[$c['id']]=$c;

if ($_POST['submit']) {
	if($_POST['remove']==1){
		$del=mysql_query("INSERT INTO event_attendance VALUES('{$_POST['clientID']}', '{$_POST['eventid']}', -1)");
		if($del) $msg='success'; else $msg='error';
		die($msg);
	} elseif($_POST['add']==1) {
		$add=mysql_query("INSERT INTO event_attendance VALUES('{$_POST['clientID']}', '{$_POST['eventid']}', 1)");
...
// $arr = array();
$s = mysql_query("SELECT * FROM clients WHERE active = 1 ORDER BY name ASC") or die(mysql_error());
// while ($sq = mysql_fetch_assoc($s)) {
	// array_push($arr,$sq['clientID']);
// }

// $arr0 = implode(",",$arr);
// print_r($arr0);
// if(mysql_num_rows($s)>0)$row=mysql_fetch_array($s);
...
$vq = mysql_query("SELECT * FROM events ORDER BY eventid");
while ($event = mysql_fetch_assoc($vq)) {
